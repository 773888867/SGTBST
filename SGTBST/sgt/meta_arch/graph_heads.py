...
from projects.SGTBST.sgt.meta_arch.loss import EdgeClsLoss, NodeClsLoss
from projects.SGTBST.sgt.meta_arch.graph_net import build_layers
...
class EdgeClsHead(nn.Module):
    ...
    def avg_edge_features(self, edge_dict):
        """
            ...
            此函数从字典中处理边特征和索引，并在提供了批次掩码的情况下计算每一批次的边特征平均值。
            它通过应用批次掩码来过滤相关边以进行平均处理，支持每批次不同数量的边。

            参数:
            - edge_dict: 包含边特征、索引以及可选批次掩码的字典。

            返回:
            - edge_features_list: 跨批次平均后的边特征列表。
            - edge_idxs: 跨批次平均后的边索引。
            """
        ...
        if batch_mask_flag:  ## 假设每批次的边数量不同
    ...
    def forward_edge_cls(self, edge_dict, mems, new_mems):
        """
           ...
           该方法计算边特征并进行前向传播以预测边的类别。
           同时计算真实标签的位置掩码。

           参数:
           - edge_dict: 包含边特征和索引信息的字典。
           - mems: 存储历史信息的字典。
           - new_mems: 存储新信息的字典。

           返回:
           - pred_logits: 预测的边类别概率。
           - edge_idxs: 边索引。
           - mask_dict: 包含各种掩码信息的字典。
           """
        edge_features_list, edge_idxs = self.avg_edge_features(edge_dict)
        # edge_dict中的features是一个tensor,而非三种特征的列表
