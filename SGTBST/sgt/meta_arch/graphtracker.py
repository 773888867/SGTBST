...
from projects.SGTBST.sgt.meta_arch.graph_net import CrossFrameInteractionGNN
from projects.SGTBST.sgt.utils import *
from projects.SGTBST.sgt.vis_utils import vis_pred_detections_by_pair
from projects.SGTBST.sgt.tracker.tracklet_manager import TrackletManager
from projects.SGTBST.sgt.meta_arch.graph_heads import NodeClsHead, EdgeClsHead
from projects.SGTBST.sgt.meta_arch.graphtracker_centernet import SGT_CenterNet_Decoder
...
class GraphTracker(nn.Module):
    ...
    def forward(self, detector_outs, batched_inputs=None):
        ...
        :param detector_outs: 从检测器输出的特征和预测结果
        :param batched_inputs: 一批输入数据，可选参数
        :return: 返回损失字典和匹配结果，如果没有有效的框，则返回零损失
        """
        # 获取节点特征图，并将其添加到输出字典中
        ...
        #det_outs['fmap'].shape = (8,64,152,272)

        # 解码检测结果，包括真实值ID和检测掩码
        decoded_detections, gt_ids, detection_batch_mask_list = self.detection_decoder.decode_training(det_outs,
                                                                                                       detector_outs)

        # 处理检测掩码，计算每个时间步的框数量
        t1_batch_mask_list, t2_batch_mask_list = detection_batch_mask_list[::2], detection_batch_mask_list[1::2]
        t1_box_nums, t2_box_nums = [b.sum().item() for b in t1_batch_mask_list], [b.sum().item() for b in
                                                                                  t2_batch_mask_list]

        # 初始化掩码，用于过滤没有检测到目标的时间步
        ...
        fmap_feats_t1, fmap_feats_t2 = decoded_detections['fmap_feat'][t1_mask], decoded_detections['fmap_feat'][
            t2_mask]
        node_feats_t1, node_feats_t2 = decoded_detections['node_feat'][t1_mask], decoded_detections['node_feat'][
            t2_mask]
        boxes_dict_t1, boxes_dict_t2 = masking_dict(decoded_detections['bboxes'], t1_mask, t2_mask)


        # split_fmap_t1, split_fmap_t2 = decoded_detections['fmap_split'][t1_mask], decoded_detections['fmap_split'][t2_mask]
        split_node_t1, split_node_t2 = decoded_detections['node_split'][t1_mask], decoded_detections['node_split'][t2_mask]

        # 准备t1和t2的信息，供GNN模块使用
        t1_info = {'tensor': {'boxes_dict': boxes_dict_t1, 'fmap_feats': fmap_feats_t1, 'node_feats': node_feats_t1,
                              'scores': scores_t1, 'gt_ids': gt_ids_t1 ,'split_node': split_node_t1},
                   'box_nums': t1_box_nums}
        t2_info = {'tensor': {'boxes_dict': boxes_dict_t2, 'fmap_feats': fmap_feats_t2, 'node_feats': node_feats_t2,
                              'scores': scores_t2, 'gt_ids': gt_ids_t2 , 'split_node': split_node_t2},
                   'box_nums': t2_box_nums}

        # 运行GNN模块，获取新的特征列表和边字典
        new_t1_feats_list, new_t2_feats_list, edge_dicts = self.gnn(t1_info, t2_info)  # 9.21改到这了
    ...
    def inference(self, detector_outs, t1_info, batched_inputs):
        ...
        # split_fmap = decoded_detections['fmap_split']
        split_node = decoded_detections['node_split']
        ...
        t2_info = {'tensor': {'boxes_dict': boxes_dict, 'fmap_feats': fmap_feats, 'node_feats': node_feats,
                              'scores': scores, 'pred_ids': pred_ids, 'split_node': split_node},
                   'box_nums': [box_nums]}
