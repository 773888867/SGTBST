...
from projects.SGTBST.sgt.nodefusion import *
...
class SGT_CenterNet_Decoder(nn.Module):
    ...
    def decode_gt_training(self, targets, det_outs):
        ...
        gt_xs_int = gt_inds % W
        # gt_ys_int = gt_inds // W # 建议下面写法
        gt_ys_int = torch.div(gt_inds, W, rounding_mode='trunc')
        ...
        for b_idx in range(B):
            ...
            # print("fmap_feature.size()",fmap_feature.size())
            # print("node_feature.size()",node_feature.size())
    ...
    def decode_topk(self, results, tlbr_flag=True):
        ...
        参数:
            results (dict): 包含模型输出张量的字典。
            tlbr_flag (bool): 标志位，指示输出格式是否为 tlbr（左上角右下角）。

        返回:
            dict: 包含解码后的前K个检测结果的字典。
        """
        K = self.topk_det
        # 断言基于当前实现的假设
        assert results['reg'] is not None and tlbr_flag  # 实现基于此假设且仅有一个类别

        # 处理热力图、回归、宽高、节点特征和 reid 特征
        hm = results["hm"].sigmoid()  # 对热力图应用 sigmoid 函数
        #↑8，11，152，272
        reg = results["reg"]  # 回归输出
        #↑8，2，152，272
        wh = results["wh"]  # 宽高输出
        #↑8，4，152，272
        node_feature = results["node"]  # 节点特征
        #↑8，64，152，272
        fmap = results["fmap"]  # 特征图
        #↑8，64，152，272
        reid_feature = results['id'] if 'id' in results else None  # 如果存在则获取 reid 特征
        detections = {}
        ...
        # 获取前K个得分、索引、类别和坐标
        scores, index,index_group, clses, ys, xs = topk_score(hm, K=K ,split_num=4)
        ...
        # 索引节点/特征图/reid 特征
        if node_feature is not None:
            node_feature = F.normalize(node_feature, dim=1)  # 归一化处理
            node_split = pipeline_node_feature(node_feature, index_group,merge=False)  # 9.20添加
            node_feature = pipeline_node_feature(node_feature, index_group,merge=True)
        if fmap is not None:
            fmap = F.normalize(fmap, dim=1)
        ...
        if fmap is not None:
            ...
            # print("detections['fmap_feat'].size()", detections['fmap_feat'].size())  # [800, 64]
        if node_feature is not None:
            detections['node_feat'] = node_feature.flatten(0, 1)
            detections['node_split'] = node_split.flatten(0, 1)
            # print("detections['node_feat'].size()", detections['node_feat'].size())  # [800, 64]
        if reid_feature is not None:
    ...
    def assign_gt_ids(self, targets, raw_detections, decoded_detections):
        """
        根据配置的标签分配度量标准，将地面真实（GT）ID分配给检测结果。

        该方法支持两种标签分配度量标准：'dist'（基于距离）和'iou'（基于交并比）。
        它首先根据配置的度量标准选择合适的算法来计算地面真实ID的分配，
        然后返回这些ID以及检测批次的掩码列表。

        参数:
        - targets: 目标对象，包含地面真实信息。
        - raw_detections: 原始检测输出，尚未经过处理。
        - decoded_detections: 解码后的检测结果，已经过处理。

        返回:
        - gt_ids: 分配给每个检测的地面真实ID列表。
        - detection_batch_mask_list: 检测批次掩码列表，指示哪些检测被分配了GT ID。

        异常:
        - 如果配置的label_assignment_metric不是'dist'或'iou'，则抛出异常。
        """
        # 根据配置的label_assignment_metric选择合适的函数来分配GT IDs
        if self.label_assignment_metric == 'dist':
            ...
            # 如果label_assignment_metric不是'dist'或'iou'，抛出异常
