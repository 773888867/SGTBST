...
class MOT2MIX():
    ...
    def gen_label_files(seq_root, label_root, seq_names):
        ...
        for seq_name in seq_names:
            ...
            # valid_flag = np.logical_and(gt[:, 6] == 1, gt[:, 7] == 1) # 修改读取多类别,之前跳过第七列1的情况
            valid_flag = (gt[:, 6] == 1)
            gt = gt[valid_flag]
            ...
            for fid in fids:
                ...
                # for _, tid, x, y, w, h, _, _, vis in curr_gt:  # lj.添加输出类别占位符
                for _, tid, x, y, w, h, _, cls, vis in curr_gt:
                    updated_tid = tid_dict[int(tid)] + tid_offset
                    x += w / 2
                    y += h / 2
                    # label_str = '0 {:d} {:.6f} {:.6f} {:.6f} {:.6f} {:.6f}\n'.format(
                    #     updated_tid, x / seq_width, y / seq_height, w / seq_width, h / seq_height, vis)  lj. 第一列设置为类别
                    label_str = '{:d} {:d} {:.6f} {:.6f} {:.6f} {:.6f} {:.6f}\n'.format(
                        int(cls), updated_tid, x / seq_width, y / seq_height, w / seq_width, h / seq_height, vis)
